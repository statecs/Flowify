...
export class ApiError extends Error {
  constructor(public status: number, message: string, public detail?: string, public data?: unknown) {
    super(message);
    this.name = 'ApiError';
  }
}
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(res.status, body.error || res.statusText, body.detail, body);
  }
